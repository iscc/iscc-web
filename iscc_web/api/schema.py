# generated by datamodel-codegen:
#   filename:  openapi.yaml

from __future__ import annotations
from pydantic import AnyUrl, BaseModel, Field, RootModel, confloat, conint, constr
from enum import StrEnum


class MediaID(RootModel[constr(pattern=r"[a-v0-9]{13}$", min_length=13, max_length=13)]):
    root: constr(pattern=r"[a-v0-9]{13}$", min_length=13, max_length=13) = Field(
        ...,
        description="ID for an uploaded file generated by server for each upload event.",
        examples=["061kcmrj55fi8"],
    )


class Iscc(RootModel[constr(pattern=r"^ISCC:[A-Z2-7]{10,73}$", min_length=15, max_length=73)]):
    root: constr(pattern=r"^ISCC:[A-Z2-7]{10,73}$", min_length=15, max_length=73) = Field(
        ...,
        description="An **ISCC** in canonical representation. This is the minimal required field for a valid ISCC Metadata object.",
        examples=["ISCC:KACYPXW445FTYNJ3CYSXHAFJMA2HUWULUNRFE3BLHRSCXYH2M5AEGQY"],
    )


class UploadResponse(BaseModel):
    content: str | None = Field(None, examples=["http://localhost:8000/api/v1/media/061knt35ejv6o"])
    media_id: MediaID | None = None


class FieldType(StrEnum):
    CreativeWork = "CreativeWork"
    TextDigitalDocument = "TextDigitalDocument"
    ImageObject = "ImageObject"
    AudioObject = "AudioObject"
    VideoObject = "VideoObject"


class BasicMetadata(BaseModel):
    field_context: AnyUrl | None = Field(
        None,
        alias="@context",
        description="The [JSON-LD](https://json-ld.org/) Context URI for ISCC metadata.",
        examples=["http://purl.org/iscc/context/0.8.0.jsonld"],
    )
    field_type: FieldType | None = Field(
        "CreativeWork",
        alias="@type",
        description="The type of digital content according to schema.org classes (TextDigitalDocument, ImageObject, AudioObject, VideoObject).",
    )
    field_schema: AnyUrl | None = Field(
        None,
        alias="$schema",
        description="The [JSON Schema](https://json-schema.org/) URI for ISCC metadata.",
        examples=["http://purl.org/iscc/schema/0.8.0.json"],
    )
    iscc: Iscc | None = None


class InlineMetadata(BaseModel):
    name: constr(max_length=128) | None = Field(
        None,
        description="The title or name of the intangible creation manifested by the identified *digital content*. **Used as input for ISCC Meta-Code generation**.",
        examples=["The Never Ending Story"],
    )
    description: constr(max_length=4096) | None = Field(
        None,
        description="Description of the *digital content* identified by the **ISCC**. **Used as input for ISCC Meta-Code generation**. Any user presentable text string (including Markdown text) indicative of the identity  of the referent may be used.",
        examples=["a 1984 fantasy film co-written and directed by *Wolfgang Petersen*"],
    )
    meta: constr(max_length=16384) | None = Field(
        None,
        description="Subject, industry, or use-case specific metadata encoded as Data-URL.",
        examples=["data:application/json;charset=utf-8;base64,eyJleHRlbmRlZCI6Im1ldGFkYXRhIn0="],
    )
    identifier: str | None = Field(
        None,
        description="Other identifier(s) referencing the work, product or other abstraction of which the referenced **digital content** is a full or partial manifestation.",
    )
    creator: str | None = Field(
        None, description="An entity primarily responsible for making the resource.", examples=["Joanne K. Rowling"]
    )
    license: str | None = Field(
        None,
        description="URI of license for the identified *digital content*.",
        examples=["https://example.com/license-terms-for-this-item"],
    )
    acquire: AnyUrl | None = Field(
        None,
        description="This field must contain a valid URL referring to a page showing information about how one can acquire a license for the item. This may be a page of a web shop or NFT marketplace ready for providing a license.",
        examples=["https://example.com/buy-license-for-item-here"],
    )
    credit: str | None = Field(
        None,
        description="A line of text that you expect users of the image (such as Google Images) to display alongside the image.",
        examples=["Frank Farian - Getty Images"],
    )
    rights: str | None = Field(
        None,
        description="Contains any necessary copyright notice and should identify the current owner of the copyright of this work with associated intellectual property rights.",
        examples=["Copyright 2022 ISCC Foundation - www.iscc.codes"],
    )
    keywords: str | list[str] | None = Field(
        None,
        description="Keywords or tags used to describe this content. Multiple entries in a keywords list are typically delimited by commas.",
    )


class Mode(StrEnum):
    text = "text"
    image = "image"
    audio = "audio"
    video = "video"
    mixed = "mixed"


class FeatureSet(BaseModel):
    maintype: str | None = Field(None, description="ISCC MainType of the simprint algorithm.", examples=["content"])
    subtype: str | None = Field(None, description="ISCC SubType of the simprint algorithm.", examples=["text"])
    version: int | None = Field(None, description="Version of the simprint algorithm.", examples=[0])
    byte_offsets: bool | None = Field(
        None, description="Whether offsets are UTF-8 byte positions instead of character positions.", examples=[False]
    )
    simprints: list[str] | None = Field(
        None,
        description="Base64-encoded granular similarity-preserving fingerprints.",
        examples=[["k5TpwXVE3j9N5IBxm36c4hkXP6fHOv8bkY2f68_8XSg"]],
    )
    offsets: list[int] | None = Field(None, description="Start positions of the features in the *digital content*.")
    sizes: list[int] | None = Field(None, description="Sizes of the features in the *digital content*.")


class SimprintRequest(BaseModel):
    text: constr(min_length=1) = Field(
        ...,
        description="Plain text to generate simprints for.",
        examples=["The quick brown fox jumps over the lazy dog."],
    )


class SimprintResponse(BaseModel):
    CONTENT_TEXT_V0: list[str] | None = Field(
        None,
        description="Base64-encoded 256-bit minhash simprints, one per content-defined text chunk.",
        examples=[["k5TpwXVE3j9N5IBxm36c4hkXP6fHOv8bkY2f68_8XSg"]],
    )
    SEMANTIC_TEXT_V0: list[str] | None = Field(
        None,
        description="Base64-encoded 256-bit semantic simprints generated by [iscc-sct](https://github.com/iscc/iscc-sct).",
        examples=[["XUj8aOg4CGm5Uosm-Ggxi7d9G9pa4NBMyBDDAhrkf00"]],
    )


class Unit(BaseModel):
    iscc_unit: str | None = Field(
        None, description="Canonical representation of ISCC-UNIT", examples=["ISCC:AAA4RJYGHHVRCZ5T"]
    )
    readable: str | None = Field(
        None, description="Human readable version of ISCC-UNIT", examples=["META-NONE-V0-64-c8a70639eb1167b3"]
    )
    hash_hex: str | None = Field(None, description="Hex representation of ISCC-BODY", examples=["e1fb7dc4e3dbb4be"])
    hash_uint: str | None = Field(
        None, description="Unsigned integer representation of ISCC-BODY", examples=["16283747162278048958"]
    )
    hash_bits: str | None = Field(
        None,
        description="Bitpattern of ISCC-BODY",
        examples=["1110000111111011011111011100010011100011110110111011010010111110"],
    )


class IsccDetail(BaseModel):
    iscc: Iscc | None = None
    readable: str | None = Field(
        None,
        description="Human readable version of ISCC",
        examples=["ISCC-VIDEO-V0-MSDI-c8a70639eb1167b367a9c3787c65c1e582e2e662f728b4fa42485e3a0a5d2f34"],
    )
    multiformat: str | None = Field(
        None,
        description="Multiformats representation base64url representation of ISCC",
        examples=["uzAFTBsinBjnrEWezZ6nDeHxlweWC4uZi9yi0-kJIXjoKXS80"],
    )
    decomposed: str | None = Field(
        None,
        description="ISCC decomomposed into a dash seperated secquence of ISCC-UNITs",
        examples=["AAA4RJYGHHVRCZ5T-CMAWPKODPB6GLQPF-GAAYFYXGML3SRNH2-IAAUESC6HIFF2LZU"],
    )
    units: list[Unit] | None = Field(None, description="Different representations of the individial units of the ISCC")


class TechnicalMetadata(BaseModel):
    mode: Mode | None = Field(None, description="The perceptual mode used to create the ISCC.", examples=["image"])
    filename: str | None = Field(
        None, description="Filename of the referenced **digital content**", examples=["your-media-file.jpg"]
    )
    filesize: int | None = Field(None, description="File size of media asset in number of bytes.")
    mediatype: str | None = Field(
        None,
        description="An [IANA Media Type](https://www.iana.org/assignments/media-types/media-types.xhtml) (MIME type)",
        examples=["image/jpeg"],
    )
    duration: int | None = Field(None, description="Duration of audio-visual media in seconds.", examples=[60])
    fps: confloat(ge=1.0) | None = Field(None, description="Frames per second of video assets.", examples=[24])
    width: int | None = Field(None, description="Width of visual media in number of pixels.", examples=[640])
    height: conint(ge=1) | None = Field(None, description="Height of visual media in number of pixels.", examples=[480])
    characters: int | None = Field(
        None, description="Number of text characters (code points after Unicode normalization)", examples=[55689]
    )
    language: str | None = Field(
        None,
        description="Primary language of content [BCP 47](https://tools.ietf.org/search/bcp47).",
        examples=["en-US"],
    )
    thumbnail: AnyUrl | None = Field(
        None,
        description="URI an autogenerated user-presentable thumbnail-image that serves as a preview of the digital content. The URI may be a Data-URL RFC2397.",
        examples=["https://picsum.photos/200/300.jpg"],
    )
    generator: str | None = Field(
        None, description="Name and version of the software that generated the ISCC.", examples=["iscc-sdk - v0.9.3"]
    )
    metahash: str | None = Field(
        None,
        description="A [multihash](https://multiformats.io/multihash/) of the metadata that was used as input for ISCC Meta-Code generation.",
        examples=["1e20304ab0c98a760be580dfa20716f2912d7ae30ec82b5f48b01dcf8f008d42568d"],
    )
    datahash: str | None = Field(
        None,
        description="A [multihash](https://multiformats.io/multihash/) of the raw bytes of the media file.",
        examples=["1e20b46b2257d6c6248f0b2130affc644088cc07f31219394f28523e7742441e91fd"],
    )
    units: list[constr(pattern=r"^ISCC:[A-Z2-7]{10,73}$")] | None = Field(
        None,
        description="ISCC-UNITs of the media asset (with `semantic=true` including the experimental Semantic-Code unit). Included by default; excluded if the service is configured with ISCC_SDK_ADD_UNITS=false.",
        examples=[
            [
                "ISCC:AAAV5NAQXBCHCWFW",
                "ISCC:CAA7X3SMP7IQ4Z65",
                "ISCC:EAAQMBEYQF6457DP",
                "ISCC:GAAYGICYTOZYKQAL",
                "ISCC:IAAUNUPL2ZHVCU3R",
            ]
        ],
    )
    features: list[FeatureSet] | None = Field(
        None,
        description="Granular features of the media asset. Included by default; excluded with `granular=false`. With `semantic=true` also includes semantic simprints.",
    )


class IsccMetadata(BasicMetadata, UploadResponse, InlineMetadata, TechnicalMetadata):
    pass
